            ...
            <div className={`mb-4 ${isCenter ? 'opacity-100' : 'opacity-70'} flex items-center justify-center h-12`}>
                {car.logo && (
                    <img
                        src={car.logo}
                        alt={car.brand}
                        className={`${isCenter ? 'max-h-12 max-w-[120px]' : 'max-h-8 max-w-[80px]'} object-contain drop-shadow-md`}
...
export default function CollectionScreen() {
    ...
    const filteredCars = cars.filter(car => {
        ...
        if (activeFilters.search && !car.brand.toLowerCase().includes(activeFilters.search.toLowerCase()) && !car.model.toLowerCase().includes(activeFilters.search.toLowerCase())) return false;
        if (activeFilters.brands?.length > 0 && !activeFilters.brands.includes(car.brand)) return false;
        if (activeFilters.bodyTypes?.length > 0 && !activeFilters.bodyTypes.includes(car.type)) return false;
        if (activeFilters.fuelTypes?.length > 0 && !activeFilters.fuelTypes.includes(car.fuelType)) return false;
        if (activeFilters.colors?.length > 0 && !activeFilters.colors.includes(car.color)) return false;
        if (activeFilters.seats?.length > 0 && !activeFilters.seats.includes(car.seats)) return false;

        if (activeFilters.transmissions?.length > 0) {
            const carTrans = car.transmission || (car.specs && car.specs.find(s => s.label === 'Transmission')?.value);
            if (carTrans && !activeFilters.transmissions.includes(carTrans)) return false;
        }

        const priceNum = parseInt(car.price?.replace(/[^\d]/g, '') || '0', 10);
        if (priceNum > activeFilters.maxPrice) return false;

        return true;
    }).sort((a, b) => {
        if (activeFilters.sort === 'price_asc') {
            return parseInt(a.price?.replace(/[^\d]/g, '') || '0', 10) - parseInt(b.price?.replace(/[^\d]/g, '') || '0', 10);
        } else if (activeFilters.sort === 'price_desc') {
            return parseInt(b.price?.replace(/[^\d]/g, '') || '0', 10) - parseInt(a.price?.replace(/[^\d]/g, '') || '0', 10);
        } else if (activeFilters.sort === 'power_desc') {
            return parseInt(b.maxPower || '0') - parseInt(a.maxPower || '0');
        }
        return 0;
    });

    const totalCars = filteredCars.length;
    const currentActiveIndex = totalCars > 0 ? Math.min(activeIndex, totalCars - 1) : 0;
    const progress = totalCars > 1 ? currentActiveIndex / (totalCars - 1) : 0;
    const activeCar = filteredCars[currentActiveIndex] || null;
    ...
    const goNext = useCallback(() => {
        setActiveIndex((prev) => {
            const current = totalCars > 0 ? Math.min(prev, totalCars - 1) : 0;
            return Math.min(totalCars - 1, current + 1);
        });
    }, [totalCars]);
    ...
    useEffect(() => {
        ...
        if (pending.carId) {
            const filteredList = cars.filter(car => targetCategory === 'All' || car.type === targetCategory);
    ...
    if (isLoading) {
        return (
            <div className="w-full min-h-screen bg-[#fafafa] flex items-center justify-center">
                <div className="text-black font-michroma text-xl animate-pulse">Loading Collection...</div>
            </div>
        );
    }
            ...
            <div className={`w-full bg-[#111111] rounded-[20px] sm:rounded-[30px] lg:rounded-[40px] flex overflow-hidden relative shrink-0 isolate h-[calc(100dvh-1rem)] sm:h-[calc(100dvh-2rem)] ${isShowingDetails ? 'min-h-[850px] lg:min-h-[640px]' : 'min-h-[640px]'}`}>
                ...
                >
                    ...
                    <LayoutGroup>
                        ...
                        <AnimatePresence>
                            {viewMode === 'carousel' && !isShowingDetails && activeCar && (
                                <>
                            ...
                            {viewMode === 'carousel' && !isShowingDetails && activeCar && (
                                <motion.div key="nav-arrows" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={FADE}>
                                    <button
                                        ...
                                        className={`absolute left-2 sm:left-4 md:left-10 top-1/2 -translate-y-1/2 z-40 text-white transition-all duration-300 ${currentActiveIndex === 0 ? 'opacity-30 cursor-not-allowed' : 'opacity-100 hover:opacity-70 hover:-translate-x-2 cursor-pointer'}`}
                                    ...
                                    <button
                                        onClick={goNext}
                                        disabled={currentActiveIndex === totalCars - 1}
                                        className={`absolute right-2 sm:right-4 md:right-10 top-1/2 -translate-y-1/2 z-40 text-white transition-all duration-300 ${currentActiveIndex === totalCars - 1 ? 'opacity-30 cursor-not-allowed' : 'opacity-100 hover:opacity-70 hover:translate-x-2 cursor-pointer'}`}
                                        aria-label="Next car"
                                    >
                                        <svg className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <AnimatePresence>
                            {viewMode === 'carousel' && filteredCars.length === 0 && !isShowingDetails && (
                                <motion.div
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    exit={{ opacity: 0 }}
                                    className="absolute inset-0 top-[11.5rem] sm:top-[10.5rem] md:top-[120px] lg:top-[140px] px-4 flex flex-col items-center justify-center z-20 pointer-events-none"
                                >
